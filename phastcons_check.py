...
def genBaseTree(scale, B):
    return BinaryTree("root", BinaryTree("CLIB382-FostersO-FostersB-VL3-CBS7960-AWRI1631-Kyokai7-M22",
        ...
        BinaryTree('FostersO'), BinaryTree('FostersB'), scale*0.001010, scale*0.000001), scale*0.000001,
        scale*0.000001), BinaryTree("VL3"), scale*0.000001, scale*0.000001), BinaryTree("CBS7960"),
        scale*0.000001,scale*0.000001), BinaryTree("AWRI1631"), scale*0.000001, scale*0.000001), 
        BinaryTree("Kyokai7"),scale*0.000001, scale*0.001907), BinaryTree("M22"), scale*0.000001, scale*0.000953),
...
def likelihoods(seqs, u_t, l, names2, max_len):
    ...
    for j in range(0,int(len(l)/2)):
        ...
        if l[2*j].getNodeValue() in order_seqs:
            ...
            cond_prob_array=[1+3*math.exp(-4*u_t*l[2*j].getrChildLength()), 
                1-math.exp(-4*u_t*l[2*j].getrChildLength()), 1+3*math.exp(-4*u_t*l[2*j].getlChildLength()),
                1-math.exp(-4*u_t*l[2*j].getlChildLength())]
            gap_data=[1,1,1,1]
...
def Forward(non_matrix, con_matrix, mu, vu):    #states is a 4x4 matrix of substitution rates
    ...
    if len(non_matrix)!=len(con_matrix):
        ...
        for i in range(1,len(non_matrix)):
            column=[]
            column.append(non_matrix[i]+sumLogProb(log(1-vu)+forward_matrix[i-1][0],log(mu)+forward_matrix[i-1][1])) #non
            column.append(con_matrix[i]+sumLogProb(log(1-mu)+forward_matrix[i-1][1],log(vu)+forward_matrix[i-1][0])) #con
...
def Backward(non_matrix, con_matrix, mu, vu):    #states is a 4x4 matrix of substitution rates
    ...
    if len(non_matrix)!=len(con_matrix):
        ...
        for i in range(1,len(non_matrix)):
            column=[]
            column.append(sumLogProb(log(1-vu)+backward_matrix[len(con_matrix)-i][0]+non_matrix[len(con_matrix)-i],log(mu)+backward_matrix[len(con_matrix)-i][1]+con_matrix[len(con_matrix)-i])) #non
            column.append(sumLogProb(log(1-mu)+backward_matrix[len(con_matrix)-i][1]+con_matrix[len(con_matrix)-i],log(vu)+backward_matrix[len(con_matrix)-i][0]+non_matrix[len(con_matrix)-i])) #con
...
def ForBack(forward_matrix, backward_matrix):
    if len(forward_matrix)!=len(backward_matrix):
        ...
        for i in range(0,len(backward_matrix)):
            denom=sumLogProb(forward_matrix[i][0]+backward_matrix[i][0],forward_matrix[i][1]+backward_matrix[i][1])
            ForBackMat.append([forward_matrix[i][0]+backward_matrix[i][0]-denom,forward_matrix[i][1]+backward_matrix[i][1]-denom])
        return ForBackMat
...
def main():
    ...
    #    genes_loc='C:\Users\Zachary_Roga\Documents\cs4775\\apoe.fa'
        ...
        mu1=2
        vu1=mu1
        while abs(mu-mu1)>0.01 or abs(vu-vu1)>0.01:
            vu1=vu
            mu1=mu
        ...
        #plt.plot(tester_data, color='g')
        for i in range(0,len(lens)):
            line1=plt.axvline(x=lens[i], color="r", lw=0.5, )
            line1.set_dashes(dashes)
        plt.xlabel("length of bp")
